...
| [`lua/corpus_cargo/server/corpus_cargo_instances.lua`](lua/corpus_cargo/server/corpus_cargo_instances.lua) | server | Blobs de instancia únicos y uid. **Sin archivo propio**: el blob viaja embebido en el archivo de su dueño y `_live` es la fuente runtime (CRG-56/57, §12). Sede del **serializador único de dueño** (`RenderOwner`/`HydrateOwner`), que comparten el record y el contenedor persistente |
...
| [`lua/corpus_cargo/server/corpus_cargo_containers.lua`](lua/corpus_cargo/server/corpus_cargo_containers.lua) | server | Contenedores en mundo, transferencias, Take/Move all (§8) — **es el primitivo inventario-en-entidad** que reusa el trader. Con `persistKey` es **archivo de dueño** (`cont_<key>`, blobs adentro, escritor único `Containers.Save`); las referencias vivas (entidad, viewers) van en `_live`, **nunca sobre la entidad** — el duplicator mergea `ent:GetTable()` entero |
...
6. **CRG-43 — Persistencia namespaced** (aplica COR-3; el remedio es a COR-8)**.** Todo vía `Corpus.Data` namespace `cargo`. Las claves, **derivadas del árbol** (§7.2: una norma que enumera sale de un grep, no de la prosa): `inv_<steamid64>`, `cont_<key>` y `trader_<key>` como estado de partida (scope `save`, el default), más los archivos de catálogo `autogen_defs` e `icon_overrides`, que **declaran `scope = "config"`** (cita **COR-19**: sobreviven a borrar una partida). Nada de `file.*` para estado propio — cita **COR-18**. **No hay clave `inst_<uid>`**: el blob de instancia viaja embebido en el archivo de su dueño (CRG-56, sede `docs/Cargo_Architecture.md` §12). Los **dos archivos de dueño** —`inv_<steamid64>` y `cont_<key>`— serializan por la MISMA rutina (`Instances.RenderOwner`/`HydrateOwner`) y llevan sus blobs adentro; `trader_<key>` guarda **solo el wallet**, porque un trader es el contenedor más una capa de precio (CRG-21) y la economía no baja al primitivo de storage. `cont_<key>` tiene **un solo escritor**, `Containers.Save`. Lo que no declaró `persistKey` no llega a disco (cita **CRG-59**). El round-trip JSON **no** preserva tipos de clave: re-normaliza claves numéricas al cargar (`Util.NumberKeys` — quick slots, tanto server como client).
