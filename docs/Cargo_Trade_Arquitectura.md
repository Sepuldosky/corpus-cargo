...
| **2 — dinero como entidad** | entidad cash, botón $ en Solo (botar) y en Trade (línea de solo-dinero) (§7) | **CHANGELOG 82 — ESCRITO 2026-08-23**, planilla **AO** esperando la pasada en juego. Dos decisiones del autor de esa fecha que el §7 no traía: el efectivo se **acota por CANTIDAD DE PROPS** y no por valor (`cargo_cash_props_max` 10, `cargo_cash_bundle_max` 1000 — con bultos de 1000 los dos números coinciden y sólo el de props acota: contando valor, diez mil bultos de $1 son diez mil props), y **NO hay timer de expiración**, porque un bulto que se evapora destruye plata en silencio mientras que una cuota puede explicarse. El modelo es de **CS:Source** (`props/cs_assault/money.mdl`, medido en su VPK) y va con gate `ModelUsable`: **GMod NO lo trae**, sólo su spawnicon |
